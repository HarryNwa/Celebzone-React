...
function App() {
  ...
  const deletExistingContact = async (event) => {
    event.preventDefault();
    try {
      const { data } = await deleteContact(values);
      const formData = new FormData();
      formData.delete('file', file, file.name);
      formData.delete('id', data.id);
      const { data: photoUrl } = await updatePhoto(formData);
      toggleModal(true);
      ...
      <dialog ref={modalRef} className="modal" id="modal">
        ...
        <div className="modal__body">
          <form onSubmit={handleNewContact}>
            ...
            {/* <div className="form_footer">
              <button onClick={() => toggleModal(false)} type='button' className="btn btn-danger">Delete</button>
