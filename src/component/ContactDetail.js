...
const ContactDetail = ({ updateContact, updateImage, deleteContact }) => {
  ...
  // const removeContact = async (file) => {
  //   try {
  ...
  //     dele
  //     disableContact((current) => ({ ...current, contact: `${current.contact}?disbled_at=${new Date().getTime()}` }));
  ...
  //   disableContact({ ...contact, [event.target.name]: event.target.value });
  // };
        ...
        <div className='profile__settings'>
          <div>
            <form onSubmit={onUpdateContact} className='form'>
              ...
              {/* <div className="form_footer">
                <button type='submit' onClick={removeContact.isDisabled} className="btn">Delete</button>
              </div> */}
            </form>
          </div>
        </div>
